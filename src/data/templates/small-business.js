    ...
    tabs: [
      ...
      },
      {
        ...
        blocks: [
          ...
          text('sb12', '', 'Drag a deal to a new day to move its expected close — on the published page that writes straight back to your Deals table. A pipeline without dates is a list, not a forecast: stages alone tell you what might close, never when. Giving every deal an expected close date is what turns the board below into a month you can actually plan against.'),
          spacer('sb12s', 10),
          ...
          text('sb20', '', 'Stage is highlighted so a deal sitting in “Cold” for a month is obvious. Ownership cascades from the company: the account owner owns its contacts and its deals, so a rep sees their accounts and everything hanging off them. That one rule is what makes per-person access workable without maintaining three separate permission lists.'),
        ],
      },
      {
      ...
      },
      {
        ...
        blocks: [
          ...
          text('sb36', '', 'Pay is hours × rate, computed rather than keyed in, so a corrected timesheet updates the cost everywhere it appears. In your own document you can add a rates table so a person’s rate can change over time and by role, and the right rate is picked automatically for each period.'),
        ],
      },
      {
        ...
        blocks: [
          ...
          text('sb43', '', 'Everyone logs their own expenses into one table instead of mailing spreadsheets around, so nobody is reconciling four versions of the same month at quarter end. Add a receipt attachment column in your own document and the photo travels with the row.'),
          spacer('sb43s', 10),
